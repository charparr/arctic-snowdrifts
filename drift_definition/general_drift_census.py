...
def compute_total_volume_for_rasters_in_dict(d):
    ...
    for k in d.keys():
        pixel_size = d[k]['meta']['transform'][0]
        total_volume = np.nansum((~np.isnan(d[k]['arr'])) * d[k]['arr'] * pixel_size)
...
def generate_test_thresholds(d, start, stop, increment):
    ...
    for k in d.keys():
        d[k]['test thresholds'] = np.round(np.arange(start, stop, increment) * d[k]['mu'], 2) 
    return d
...
def recompute_from_new_threshold(d):
    ...
    for k in d.keys():
        ...
        exceeding_depth_values[exceeding_depth_values == 0] = np.nan
        # Compute Area Exceeding Threshold (i.e. count number of pixels), pixel size=1
        d[k]['total snowdrift area m^2'] = int(np.nansum(exceeding_depth_values))
        d[k]['pct. snowdrift area'] = (d[k]['total snowdrift area m^2'] / d[k]['total area m^2']) * 100
        # Compute Volume Exceeding Threshold (i.e. sum of pixel intensities), pixel size=1
        d[k]['total snowdrift volume m^3'] = np.nansum(exceeding_depth_values * d[k]['arr'])
...
if __name__ == '__main__':
	...
	for k in snow_d.keys():
		...
		print('Area Normalized Snowdrift Susceptibility [m]: %f' % snow_d[k]['area normalized snowdrift volume m^3/m^2'])
		print('---')

	for k in snow_d.keys():
		print(snow_d[k]['swath'], snow_d[k]['year'], snow_d[k]['total area m^2'] / 1000000)
		print('---')
	...
	# Then recompute
	clpx_s[os.path.join(clpx_s_dir, 'clpx_2015_s_end_depth.tif')]['snowdrift depth threshold m'] = 0.82
	clpx_s[os.path.join(clpx_s_dir, 'clpx_2016_s_end_depth.tif')]['snowdrift depth threshold m'] = 0.65
	clpx_s[os.path.join(clpx_s_dir, 'clpx_2017_s_end_depth.tif')]['snowdrift depth threshold m'] = 0.75
	...
	adjusted_df['Drift Volume / Drift Area (Avg. Drift Depth) $m$']
